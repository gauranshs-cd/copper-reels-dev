-- Clean Supabase Schema for Copper Reels
-- This version creates tables first, then adds indexes to avoid column reference errors
...
DROP TABLE IF EXISTS public.video_plans CASCADE;
DROP TABLE IF EXISTS public.thumbnail_briefs CASCADE;
...
DROP TABLE IF EXISTS public.creative_briefs CASCADE;
DROP TABLE IF EXISTS public.sessions CASCADE;
DROP TABLE IF EXISTS public.pattern_banks CASCADE;
DROP TABLE IF EXISTS public.pillars CASCADE;
DROP TABLE IF EXISTS public.audience_avatars CASCADE;
DROP TABLE IF EXISTS public.channels CASCADE;
DROP TABLE IF EXISTS public.profiles CASCADE;
*/

-- Create all tables first (without indexes)
...
DROP TRIGGER IF EXISTS update_audience_avatars_updated_at ON public.audience_avatars;
...
DROP TRIGGER IF EXISTS update_pattern_banks_updated_at ON public.pattern_banks;
CREATE TRIGGER update_pattern_banks_updated_at BEFORE UPDATE ON public.pattern_banks
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_creative_briefs_updated_at ON public.creative_briefs;
...
DROP TRIGGER IF EXISTS update_video_plans_updated_at ON public.video_plans;
...
-- Create a simple policy for generation_logs (users can insert but not read)
CREATE POLICY "Users can insert generation logs" ON public.generation_logs
  FOR INSERT WITH CHECK (true);

-- Success message
DO $$
BEGIN
  RAISE NOTICE 'Schema created successfully! All tables, indexes, and policies are in place.';
END $$;